 ...
 * 데이터를 웹브러우저에 응답한 후 서버는 웹브라우저와의 관계를 종료한다.
 * 연결이 끊어졌을 때 어떤 정보를 지속적으로 유지하기 위한 수단으로 쿠키라는 방식을 사용한다. 
 * 쿠키는 서버에서 생성하여 서버가 아닌 클라이언트측에 특정 정보를 저장한다. (4KB 용량제한)
 * 
 * 세션도 쿠키와 마찬가지로 서버와으이 관계를 유지하기 위한 수단입니다. 
 * 단, 쿠키와 달리 클라이언트의 특정 위치에 저장되는 것이 아니라, 서버상의 객체로 존재한다. 
...
public class Example10_SessionSet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//현재 session이 존재하면 기존 session을 리턴하고, 존재하지 않으면 새로운 session을 만들어 준다.
